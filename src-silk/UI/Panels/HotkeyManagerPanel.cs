...
namespace eft_dma_radar.Silk.UI.Panels
{
    ...
    internal static class HotkeyManagerPanel
    {
        ...
        {
            bool isOpen = IsOpen;
            using var scope = PanelWindow.Begin("\u2328 " + Chinese.H("Hotkeys"), ref isOpen, new Vector2(460, 400));
            IsOpen = isOpen;
            if (!scope.Visible)
                return;
            ...
            if (!InputManager.IsReady)
            {
                ImGui.TextColored(new Vector4(1f, 0.6f, 0.2f, 1f),
                    "\u26a0 " + Chinese.H("Input manager not initialized."));
                ImGui.TextWrapped(Chinese.H("Hotkeys require an active DMA connection. They will activate once a raid starts."));
                return;
            }
            ...
            ImGui.TextColored(new Vector4(0.6f, 0.6f, 0.6f, 1f),
                Chinese.H("Hotkeys work via DMA — they read the gaming PC's keyboard state."));
        }
        ...
        {
            ImGui.SeparatorText(Chinese.H("Add Hotkey"));
            ...
            {
                ImGui.TextColored(new Vector4(0.6f, 0.6f, 0.6f, 1f), Chinese.H("All actions have hotkeys assigned."));
                return;
            }

            // 1. Action dropdown
            ImGui.SetNextItemWidth(200);
            if (ImGui.Combo(Chinese.H("Action"), ref _selectedActionIndex, _unboundNames, _unboundNames.Length))
            {
            ...
            string keyLabel = _isCapturing
                ? Chinese.H("[ Press a key... ]")
                : _capturedVk > 0 ? VK.GetName(_capturedVk) : Chinese.H("(None)");
            ...
            if (ImGui.IsItemHovered())
                ImGui.SetTooltip(_isCapturing ? Chinese.H("Press a key to bind, or Escape to cancel") : Chinese.H("Click to capture a key"));

            // 3. Mode radio buttons
            ImGui.RadioButton(Chinese.H("Toggle"), ref _selectedMode, 0);
            ImGui.SameLine();
            ImGui.RadioButton(Chinese.H("OnKey (Hold)"), ref _selectedMode, 1);
            ...
            if (ImGui.Button("\u2713 " + Chinese.H("Add Hotkey"), new Vector2(120, 0)) && canAdd)
            {
        ...
        {
            ImGui.SeparatorText(Chinese.H("Active Hotkeys"));

            var hotkeys = SilkProgram.Config.Hotkeys;
            if (hotkeys.Count == 0)
            {
                ImGui.TextColored(new Vector4(0.6f, 0.6f, 0.6f, 1f), Chinese.H("No hotkeys configured."));
                return;
            }
            ...
            {
                ImGui.TableSetupColumn(Chinese.H("Action"), ImGuiTableColumnFlags.None, 3f);
                ImGui.TableSetupColumn(Chinese.H("Key"), ImGuiTableColumnFlags.None, 2f);
                ImGui.TableSetupColumn(Chinese.H("Mode"), ImGuiTableColumnFlags.None, 1.5f);
                ImGui.TableSetupColumn(Chinese.H("Category"), ImGuiTableColumnFlags.None, 2f);
                ...
                foreach (var (id, entry) in hotkeys)
                {
                    ...
                    ImGui.TableNextColumn();
                    ImGui.Text(entry.Mode == HotkeyMode.Toggle ? Chinese.H("Toggle") : Chinese.H("OnKey"));
                    ...
                    if (ImGui.SmallButton($"\u2715##{id}"))
                        toRemove = id;
                    if (ImGui.IsItemHovered())
                        ImGui.SetTooltip(Chinese.H("Remove this hotkey"));
                }
