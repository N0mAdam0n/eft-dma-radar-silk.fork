...
namespace eft_dma_radar.Silk.UI.Panels
{
    ...
    internal static class PlayerHistoryPanel
    {
        ...
        {
            bool isOpen = IsOpen;
            using var scope = PanelWindow.Begin("[H] 玩家历史", ref isOpen, new Vector2(620, 450));
            IsOpen = isOpen;
            if (!scope.Visible)
                return;
            ...
            if (history is null)
            {
                ImGui.TextColored(ColGrey, Chinese.T("Player history not available."));
                return;
            }
        ...
        private static void DrawToolbar(PlayerHistory history)
        {
            ...
            ImGui.InputTextWithHint("##histSearch", Chinese.T("Search..."), ref _searchText, 128);
            ImGui.SameLine();
            ImGui.TextColored(ColGrey, $"{history.Count} " + Chinese.T("entries"));
            ImGui.SameLine();
            if (ImGui.Button(Chinese.T("Clear All")))
            {
                history.Clear();
                InvalidateCache();
            }
