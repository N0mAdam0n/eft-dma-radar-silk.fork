...
namespace eft_dma_radar.Silk.UI.Widgets
{
    ...
    internal static class LootWidget
    {
        ...
        public static void Draw()
        {
            ...
            using var scope = PanelWindow.Begin("物资", ref isOpen, new Vector2(480, 360));
            IsOpen = isOpen;
            if (!scope.Visible)
                return;
            ...
            if (visibleCount == 0)
            {
                ImGui.TextColored(UITheme.Dim, Chinese.L("No loot matches current filters"));
                return;
            }
        ...
        private static void DrawSummary(int visible, long totalValue, int total)
        {
            ...
            if (totalValue > 0)
            {
                ...
                ImGui.SameLine();
                ImGui.TextColored(UITheme.Grey, Chinese.L("total value"));
                ImGui.SameLine();
            }

            // Right-aligned: count
            string countText = total > 0 ? $"{visible}/{total}" : "0";
            float textWidth = ImGui.CalcTextSize(countText).X + ImGui.CalcTextSize(Chinese.L(" items")).X;
            ...
            ImGui.SameLine(0, 0);
            ImGui.TextColored(UITheme.Dim, Chinese.L(" items"));
        }
        ...
        private static void DrawPingBar()
        {
            ...
            ImGui.TextColored(UITheme.Magenta, $"◎ {LootPing.Count} " + Chinese.L("pinged"));
            ImGui.SameLine();
            if (ImGui.SmallButton(Chinese.L("Clear pings") + "##pings"))
                LootPing.Clear();
            if (ImGui.IsItemHovered())
                ImGui.SetTooltip(Chinese.L("Remove all loot pings"));
            ImGui.SameLine();
            ImGui.TextColored(UITheme.Dim, Chinese.L("highlighted on radar"));
            ImGui.Separator();
        }
        ...
        private static void DrawTable()
        {
            ...
            ImGui.TableSetupColumn(Chinese.L("Item"),  ImGuiTableColumnFlags.WidthStretch, 0f, 0);
            ImGui.TableSetupColumn(Chinese.L("Price"), ImGuiTableColumnFlags.WidthFixed | ImGuiTableColumnFlags.DefaultSort | ImGuiTableColumnFlags.PreferSortDescending, 65f, 1);
            ImGui.TableSetupColumn(Chinese.L("Qty"),   ImGuiTableColumnFlags.WidthFixed | ImGuiTableColumnFlags.PreferSortDescending, 32f, 2);
            ImGui.TableSetupColumn(Chinese.L("Total"), ImGuiTableColumnFlags.WidthFixed | ImGuiTableColumnFlags.PreferSortDescending, 65f, 3);
            ImGui.TableSetupColumn(Chinese.L("Dist"),  ImGuiTableColumnFlags.WidthFixed, 42f, 4);
